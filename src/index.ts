...
interface PluginTool {
  ...
  execute: (
    args: any,
    context: { client: any; sessionID?: string },
  ) => Promise<any>;
}
...
class OpenCodePGMemoryPlugin {
  ...
  private mergeConfig(userConfig: Partial<PluginConfig>): PluginConfig {
    return {
      ...
      embedding: {
        ...DEFAULT_PLUGIN_CONFIG.embedding,
        ...userConfig.embedding,
      },
      cache: { ...DEFAULT_PLUGIN_CONFIG.cache, ...userConfig.cache },
      reflection: {
        ...DEFAULT_PLUGIN_CONFIG.reflection,
        ...userConfig.reflection,
      },
      tokenBudget: {
        ...DEFAULT_PLUGIN_CONFIG.tokenBudget,
        ...userConfig.tokenBudget,
      },
      retrieval: {
        ...DEFAULT_PLUGIN_CONFIG.retrieval,
...
export const OpenCodePGMemory: Plugin = async (ctx: PluginContext) => {
  ...
  const eventSync = new EventSynchronizer(pool, {
    mode: (process.env.PG_MEMORY_SYNC_MODE || "hybrid") as SyncMode,
  ...
  if (process.env.PG_MEMORY_DB_POLLING !== "false") {
    ...
    dbPolling
      .start()
      .catch((err) => logger.warn("DB polling start failed", err));
  }
      ...
      hindsight_reflect: {
        ...
        execute: async (
          ...
          return hindsightReflect(args, pool, {
            ...
            modelSize: plugin.config.reflection.modelSize as
              | "7b"
              | "14b"
              | "full",
...
function formatMemoryContext(facts: RetrievedFact[]): string {
  ...
  for (const f of facts) {
    ...
    const score = f.relevanceScore
      ? ` (${(f.relevanceScore * 100).toFixed(0)}%)`
      : "";
    lines.push(
      `- [${typeLabel}${tierLabel}] ${f.content.substring(0, 200)}${score}`,
    );
  }
  lines.push(
    "",
    'Tip: Use recall_memory(query="...") to search more specific memories, or /pg-memory-reflect to summarize this session.',
  );
  return lines.join("\n");
}
