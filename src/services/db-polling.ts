...
export class OpenCodeDBPollingSource {
  ...
  constructor(
    ...
    config?: Partial<DBPollingConfig>,
  ) {
    this.pool = pool;
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.warn("Failed to load known sessions from DB polling start");
    }
  ...
  private async sync(): Promise<void> {
    ...
    try {
      ...
      for (const s of sessions) {
        if (!this.knownSessions.has(s.id)) {
          ...
          await this.synchronizer.handleEvent(
            this.event("session.created", s.id, {
              title: s.title,
              agent: s.agent,
            }),
          );
      ...
      if (totalToolCalls > 0) {
        logger.info("Batch syncing tool calls", {
          total: totalToolCalls,
          batchSize: BATCH_SIZE,
          lastSyncTime: this.lastSyncTime,
        });
        for (let offset = 0; offset < totalToolCalls; offset += BATCH_SIZE) {
          const toolCalls = this.adapter.getToolCallsSince(
            since,
            BATCH_SIZE,
            offset,
          );
          ...
          logger.info("Batch progress", {
            offset: offset + toolCalls.length,
            total: totalToolCalls,
          });
        }
      }
