 ...
 *
 * Usage: node scripts/verify-embedder.js
...
const { initAsyncEmbedder, getAsyncEmbedder } = require('../dist/src/services/async-embedder');
...
async function main() {
  ...
  embedder.enqueue('observations', testId, 'AsyncEmbedder verification test', 3);
  ...
  const check = await pool.query('SELECT embedding IS NOT NULL as has_emb FROM observations WHERE id = $1', [testId]);
  const hasEmb = check.rows[0]?.has_emb;

  console.log(`  embedding IS NULL? ${!hasEmb} | IS NOT NULL? ${hasEmb} ${hasEmb ? '✅' : ''}`);

  if (hasEmb) {
    const dim = await pool.query('SELECT length(embedding::text) as len FROM observations WHERE id = $1', [testId]);
    console.log(`  Embedding stored (${dim.rows[0].len} chars)`);
    console.log('\n=== ASYNC EMBEDDER TEST PASSED ✅ ===');
    console.log('  (AsyncEmbedder successfully generated and wrote embedding to PG)');
  } else {
    ...
    console.log('  ✗ AsyncEmbedder did not write embedding. Likely vector format issue.');
    console.log('  (pg driver serializes arrays as PG array format {...}, but vector expects [...])');
    console.log();
    console.log('  Attempting direct write with correct vector format...');
    
    try {
      const emb = await svc.generateEmbedding('AsyncEmbedder verification test');
      ...
      const check2 = await pool.query('SELECT embedding IS NOT NULL as has_emb FROM observations WHERE id = $1', [testId]);
      if (check2.rows[0]?.has_emb) {
        console.log('  ✓ Direct write succeeded (vector format: JSON.stringify/[] style)');
        console.log();
        console.log('=== ASYNC EMBEDDER TEST FAILED ❌ (direct write WORKS though) ===');
        console.log('NOTE: The AsyncEmbedder source has a vector serialization bug.');
        console.log('It passes JS arrays directly to $1::vector, but the pg driver');
        console.log('serializes them as {...} (PG array format) instead of [...] (vector format).');
        console.log();
        console.log('Fix: Use pgvector.toSql(embedding) or format as string before passing to PG.');
