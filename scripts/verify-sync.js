 ...
 * 
 * Usage: node scripts/verify-sync.js [--json]
 */
...
// ── Config ──
const SQLITE_PATH = path.join(os.homedir(), '.local', 'share', 'opencode', 'opencode.db');
const PG_CONFIG = {
...
function getSQLiteCounts() {
  ...
  try {
    const sessions = db.prepare('SELECT count(*) as cnt FROM session').get()?.cnt || 0;
    const messages = db.prepare('SELECT count(*) as cnt FROM message').get()?.cnt || 0;
    const parts = db.prepare('SELECT count(*) as cnt FROM part').get()?.cnt || 0;
    ...
    return { error: 'SQLite query failed: ' + e.message, sessions: 0, messages: 0, parts: 0, toolParts: 0, sessionsData: [] };
  } finally {
    db.close();
  }
}
...
async function getPGCounts() {
  ...
  try {
    const obsResult = await pool.query('SELECT count(*)::int as cnt FROM observations');
    const observations = obsResult.rows[0]?.cnt || 0;
    
    const withToolCall = await pool.query("SELECT count(*)::int as cnt FROM observations WHERE tool_call_id IS NOT NULL");
    ...
    const embResult = await pool.query(`
      ...
      FROM observations
    `);
    const emb = embResult.rows[0] || { total: 0, with_emb: 0, null_emb: 0, null_pct: 0 };
    
    return { observations, obsWithToolCallId, bySession: bySession.rows, byTool: byTool.rows, toolStatuses: toolStatuses.rows, embedding: emb, error: null };
  } catch (e) {
    return { error: 'PG query failed: ' + e.message, observations: 0, obsWithToolCallId: 0, bySession: [], byTool: [], toolStatuses: [] };
  } finally {
    await pool.end();
  }
}
...
async function main() {
  ...
  const missingSessions = sqlite.sessionsData ? sqlite.sessionsData.filter(s => {
    // Check if this session has observations
    const hasObs = pg.bySession ? pg.bySession.some(o => o.opencode_session_id === s.session_id) : false;
    return !hasObs;
  }).length : 0;
  
  if (isJson) {
    console.log(JSON.stringify({
      sqlite: { sessions: sqlite.sessions, messages: sqlite.messages, parts: sqlite.parts, toolParts: sqlite.toolParts },
      pg: { observations: pg.observations, withToolCallId: pg.obsWithToolCallId, sessionsWithObservations: pg.bySession?.length || 0 },
      embedding: pg.embedding || { total: 0, with_emb: 0, null_emb: 0, null_pct: 0 },
  ...
  console.log(`  Missing tool_call_id:       ${pg.observations - pg.obsWithToolCallId}`);
  ...
  if (pg.embedding) {
    ...
    console.log(`  With embedding:  ${pg.embedding.with_emb} / ${pg.embedding.total} (${(100 - pg.embedding.null_pct).toFixed(2)}%)`);
    if (warn) console.log(`  ⚠️  ${pg.embedding.null_emb} observations missing embedding (>5% threshold)`);
    console.log();
  }
  ...
  if (pg.bySession && pg.bySession.length > 0) {
    ...
    for (const s of pg.bySession.slice(0, 20)) {
      ...
      const sqliteSession = sqlite.sessionsData?.find(ss => ss.session_id === s.opencode_session_id);
      const toolCalls = sqliteSession?.cnt || '?';
      const pct = toolCalls !== '?' && toolCalls > 0 ? ((s.cnt / toolCalls) * 100).toFixed(1) + '%' : '?%';
      console.log(`  ${s.opencode_session_id.substring(0, 30)}...  tools:${toolCalls}  obs:${s.cnt}  (${pct})`);
    }
    console.log();
  }
}
