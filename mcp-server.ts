...
const TOOLS: Tool[] = [
  ...
  },
  {
      ...
      description: '对会话/任务/话题段的观察进行反思，归纳经验模式，生成可复用的反思记录。',
      inputSchema: {
        type: 'object',
        properties: {
          session_id: {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
       if (name === 'recall_memory') {
        ...
        const input = args as unknown as RecallMemoryInput;
        const result = await recallMemory(input, pool);
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  success: false,
                    error: 'Missing required parameter: one of session_id, omo_task_id, or topic_segment_id'
                })
              }
            ],
            isError: true
